...
- [x] **Live scan progress via Tauri events**: backend command `scan_folder` accepts `AppHandle` and forwards `ops::scan_library` log lines via `"scan-progress"` events. Frontend listens, parses log types, and presents a beautiful scrolling terminal logs modal with live-updating stat counters.
